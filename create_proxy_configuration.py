...
var config = {
        ...
        pacScript : 
            {
                url : "http://proxy.bg.pwr.wroc.pl/proxy.pac"
            }
    };
...
def create_proxy_configuration(login, password, output):
    with zipfile.ZipFile(output, 'w') as zp:
        zp.writestr("manifest.json", manifest_json)
        zp.writestr("background.js", background_js % (login, password))
...
def main(args=None):
    ...
    create_proxy_configuration(a.proxy_login, a.proxy_password, a.output_proxy)
